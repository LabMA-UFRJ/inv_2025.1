...
-- SUBCONSULTA PARA DADOS DA AT_INV
    WITH CTE_AT_INV AS
        ...
        SELECT REF_INFO, EMP, COD_EMP, COBERTURA, COUNT(*) AS QTDE_AT_CPF_UNI, PRODUTO,
            -- CALCULO ESTOQUES
        ...
        GROUP BY REF_INFO, EMP, COD_EMP, CPF, COBERTURA, PRODUTO
        )  ATIVOS_INV_BASE

        GROUP BY REF_INFO, EMP, COD_EMP, COBERTURA,             ROUND((2 * REF_INFO - EXTRACT(YEAR FROM MAX_DATA_NASC)- EXTRACT(YEAR FROM MIN_DATA_NASC)) / 2)
