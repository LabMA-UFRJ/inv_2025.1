...
-- SUBCONSULTA PARA DADOS DA AT_INV
    WITH CTE_AT_INV AS
        ...
        GROUP BY REF_INFO, EMP, COD_EMP, CPF, COBERTURA, PRODUTO, DATA_INGR, CASE WHEN SEXO NOT IN (N'M', N'F', N'm', N'f') THEN N'UNI' ELSE SEXO END
        ) ATIVOS_INV_BASE

        GROUP BY REF_INFO, EMP, COD_EMP, COBERTURA, ROUND((2 * REF_INFO - EXTRACT(YEAR FROM MAX_DATA_NASC)- EXTRACT(YEAR FROM MIN_DATA_NASC)) / 2), PRODUTO, SEXO),

    -- SUBCONSULTA PARA DADOS DA SA_INV
    CTE_SA_INV AS
        (-- SELECIONO AS INFORMAÇÕES NECESSARIAS DA TABELA DE SAIDA JÁ AGRUPADA POR CPF UNICO
        SELECT REF_INFO, EMP,  COD_EMP, COBERTURA, PRODUTO, COUNT(*) AS QTDE_SA_CPF_UNI, SEXO,
        ...
        WHERE LINHA_VALIDA = 1
        GROUP BY REF_INFO, EMP, COD_EMP, CPF, COBERTURA, MOT_SAIDA, PRODUTO, CASE WHEN SEXO NOT IN (N'M', N'F', N'm', N'f') THEN N'UNI' ELSE SEXO END
        )  SAIDAS_INV_BASE

        GROUP BY REF_INFO, EMP, COD_EMP, COBERTURA, PRODUTO, SEXO, ROUND((2 * REF_INFO - EXTRACT(YEAR FROM MAX_DATA_NASC)- EXTRACT(YEAR FROM MIN_DATA_NASC)) / 2))
